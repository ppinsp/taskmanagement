    ...
    <v-data-table
      ...
      <template v-slot:[`item.option`]="{ item }">
        <v-menu>
          ...
          <v-list>
            <v-list-item @click="$router.push({ name: 'DepartmentDetail', params: { id: item.id }})">
              <v-list-item-title>Detail</v-list-item-title>
            </v-list-item>
            <v-list-item @click="$router.push({ name: 'UpdateDepartment', params: { id: item.id }})">
  ...
  methods: {
    ...
    async getData () {
      ...
      try {
        ...
        const { data } = await userService.getAllDepartments(query)
